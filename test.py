...
def fit_text(draw, text, box_width, box_height, font_path, is_vertical=False):
...
def draw_vertical_text_rtl(draw, lines, font, box_x, box_y, box_width, box_height, text_color="white"):
...
for item in results:
    ...
    # 最適なフォントサイズと配置データを取得
    best_data, custom_font = fit_text(draw, text, width, height, font_path, is_vertical)
    
    if is_vertical:
        # 縦書きの場合（best_dataは列のリスト）
        draw_vertical_text_rtl(draw, best_data, custom_font, xmin, ymin, width, height, text_color=text_color)
    else:
        # 横書きの場合（best_dataは改行済みの文字列）
        bbox = draw.multiline_textbbox((0, 0), best_data, font=custom_font, spacing=4)
